<dialog id="modalEmail{{ $getTrainerData->id }}">
    <h2 class="poppins-bold">{{ $getTrainerData->nama }}</h2>
    <form id="formEmail{{ $getTrainerData->id }}">
        @csrf
        <div class="voucher-code-container-admin">
            <h6 class="voucher-code-title-admin">Add/Edit Email</h6>
            <input type="email" id="pinInput" name="email" value="{{ $getTrainerData->email }}"
                class="voucher-input p-4" required>
        </div>
        <button type="submit" class="voucher-button-admin">Create</button>
    </form>
    <button onclick="window.modalEmail{{ $getTrainerData->id }}.close();" aria-label="close" class="x">❌</button>
</dialog>
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script>
    $(document).ready(function() {
        $('#formEmail{{ $getTrainerData->id }}').submit(function(e) {
            e.preventDefault();
            ...
            $.ajax({
                url: '{{ url('/dataTrainer/private/email/') }}/' + nama,
                type: 'POST',
                data: formData,
